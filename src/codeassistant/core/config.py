"""Configuration management for CodeAssistant.
...
class CodeAssistantConfig:
    """Global configuration for CodeAssistant.
    ...
    def from_cli(
        ...
        **kwargs,
        ...
        config.provider = (
            provider
            or os.getenv("CODEASSISTANT_PROVIDER")
            or config.provider
        )
        config.model = (
            model
            or os.getenv("CODEASSISTANT_MODEL")
            or config.model
        )
        config.api_key = (
            api_key
            or os.getenv("CODEASSISTANT_API_KEY")
        ...
        config.api_base = (
            api_base
            or os.getenv("CODEASSISTANT_API_BASE")
            or os.getenv("OPENAI_API_BASE")
            or config.api_base
        )
        config.permission_mode = (
            permission_mode
            or os.getenv("CODEASSISTANT_PERMISSION_MODE")
    ...
    def _load_from_file(self) -> None:
        """Load configuration from ~/.codeassistant/config.yaml."""
        import yaml
    ...
    def save_to_file(self) -> None:
        """Save current configuration to ~/.codeassistant/config.yaml."""
        import yaml
        config_path = os.path.expanduser("~/.codeassistant/config.yaml")
