"""Engine orchestrator - wires together all MAHE subsystems.
...
from mahe.core.agent import ReActAgent
...
from mahe.llm.litellm_adapter import LiteLLMAdapter
from mahe.tools.base import ToolRegistry
from mahe.tools.file_tools import ReadFile, WriteFile, EditFile, GlobTool, GrepTool
from mahe.tools.shell_tools import BashTool
from mahe.tools.git_tools import GitStatus, GitDiff, GitLog, GitBranch, GitAdd, GitCommit
from mahe.tools.code_tools import AnalyzeCode, PythonDefinition, PythonReferences, PythonHover
from mahe.tools.memory_tools import SaveMemory, RecallMemory, ListMemories
from mahe.tools.test_tools import RunTests
from mahe.tools.subagent_tools import DelegateTask
from mahe.context.compression import ContextCompressor
from mahe.context.persistent import PersistentMemory
from mahe.core.subagent import SubAgentManager, SubAgentConfig
from mahe.utils.permissions import PermissionChecker, PermissionPolicy
from mahe.utils.sandbox import Sandbox

logger = logging.getLogger("mahe.engine")


class Engine:
    """Top-level orchestrator for MAHE.
    ...
    Lifecycle:
    1. Created with a MaheConfig and working directory
    2. process_query() is called for each user input
    3. reset_session() clears conversation history
    """

    def __init__(self, config: MaheConfig, working_dir: str):
        self.config = config
    ...
    def update_config(self, config: MaheConfig) -> None:
