...
def ask(ctx, model, working_dir, query):
    ...
    Example: mahe ask "what does git status do?"
    """
    query_str = " ".join(query)
    if not query_str:
        console.print("[red]Error:[/] Please provide a query. Usage: mahe ask <query>")
        sys.exit(1)

    from mahe.core.config import MaheConfig
    from mahe.core.engine import Engine
...
def config(ctx, list_config, set_key):
    """View and manage MAHE configuration.

    Example: mahe config --list
             mahe config --set model gpt-4o
    """
    from mahe.core.config import MaheConfig

    if list_config:
        cfg = MaheConfig.from_cli()
        console.print("[bold]MAHE Configuration:[/]")
        ...
        console.print("Usage: mahe config --list | mahe config --set <key> <value>")
...
def index_cmd(ctx, update, force, working_dir):
    ...
    try:
        ...
        from mahe.context.vector_store import VectorStore
        ...
        store = VectorStore(
            persist_dir=os.path.join(wd, ".mahe", "vectors"),
...
def mcp_list():
    ...
    import yaml
    config_path = os.path.expanduser("~/.mahe/mcp_servers.yaml")
...
def mcp_serve(ctx):
    ...
    """
    from mahe.tools.mcp_client import MCPClient
...
def loop_start(ctx, interval, prompt):
    ...
    from mahe.core.config import MaheConfig
    from mahe.core.engine import Engine
    from mahe.core.loop import LoopConfig, LoopScheduler
...
def loop_list(ctx):
    ...
    console.print("Use [bold]mahe chat[/] and then [bold]/loop list[/] to see active loops.")
...
def loop_once(ctx, delay, prompt):
    ...
    from mahe.core.config import MaheConfig
    from mahe.core.engine import Engine
    from mahe.core.loop import LoopScheduler
