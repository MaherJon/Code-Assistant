"""Interactive REPL loop for MAHE."""
...
from mahe.cli.renderer import Renderer
...
class REPL:
    """Interactive Read-Eval-Print Loop for MAHE.

    Features:
    - Multi-line input (Alt+Enter for newline)
    - Command history persisted to ~/.mahe/history
    ...
    COMMANDS = {
        ...
        "/exit": "Exit MAHE (also /quit, Ctrl+D)",
    ...
    def __init__(self, config: MaheConfig, working_dir: str):
        ...
        history_dir = os.path.expanduser("~/.mahe")
    ...
    def _handle_loop_command(self, args: str) -> None:
        ...
        if subcmd == "start":
            ...
            from mahe.core.loop import LoopConfig
        ...
        while self._running:
            try:
                user_input = await self.session.prompt_async(
                    [("class:prompt", "mahe"), ("class:separator", "> ")],
                )
    ...
    def _check_api_key(self) -> None:
        ...
        if not self.config.api_key:
            ...
            env_keys = ["MAHE_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY"]
            for key in env_keys:
                if os.getenv(key):
                    return
            ...
            self.renderer.info("  MAHE_API_KEY, OPENAI_API_KEY, or ANTHROPIC_API_KEY")
            self.renderer.info("Or use: mahe config --set api_key <your-key>")
            self.renderer.info("")
